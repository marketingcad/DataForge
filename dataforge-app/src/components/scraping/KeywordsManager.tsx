...
export function KeywordsManager({ initial }: KeywordsManagerProps) {
  ...
  // Grid vs list view — persisted to localStorage
  ...
  async function handleRunNow(kwId: string) {
    ...
    // timed out before writing "completed"), do one final check.
  ...
  function applyJobResult(kwId: string, jobId: string, job: { status: string; leadsDiscovered: number; leadsProcessed: number; duplicatesFound: number; errorMessage: string | null }) {
              ...
              jobs: [
                {
                  ...
                  duplicatesFound: job.duplicatesFound ?? 0,
    ...
    if (job.status === "failed") {
      ...
      const dupNote = (job.duplicatesFound ?? 0) > 0 ? ` · ${job.duplicatesFound} already existed` : "";
      setRunToast({ id: kwId, msg: `Done! ${job.leadsProcessed} leads saved to the database${dupNote}.`, ok: true });
    } else if (job.leadsDiscovered > 0) {
      setRunToast({ id: kwId, msg: `Done! All ${job.leadsDiscovered} leads were already in the database.`, ok: true });
    } else {
