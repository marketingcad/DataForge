...
export async function scrapeGoogleMapsHeadless(
  ...
  // Scales every pacing delay. Boost ≈ 25% of normal (very fast, higher block
  // risk); even without boost we run at ~60% of the old delays to cut
  // function-time/cost. Floors keep a little jitter so it isn't perfectly robotic.
  const paced = (ms: number) =>
    boost ? Math.max(40, Math.round(ms * 0.25)) : Math.max(120, Math.round(ms * 0.6));
  ...
  try {
    ...
    while (leads.length < maxLeads && staleRounds < 4) {
      ...
      for (const child of childDivs) {
        ...
        // ── Turbo card-mode (boost only) ──────────────────────────────────────
        // Read phone / address straight from the list card. If the card already
        // exposes a phone, emit the lead WITHOUT opening the (slow) detail panel.
        // Trades some completeness (full address / city / state / email that only
        // the panel shows) for a big speed win — hence boost-only.
        if (boost) {
          const card = await article.evaluate((el: HTMLElement) => {
            const text = el.innerText || "";
            const pm = text.match(/(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/);
            const phone = pm ? pm[0].trim() : undefined;

            let address: string | undefined;
            const lines = text.split("\n").map((l) => l.trim()).filter(Boolean);
            for (const line of lines) {
              const seg = line.includes("·") ? line.split("·").pop()!.trim() : line;
              if (/^\d+(\.\d+)?\s*\(/.test(seg)) continue;          // rating "(123)"
              if (/^(Open|Clos|Opens|Permanently)/i.test(seg)) continue; // hours
              if (/\d/.test(seg) &&
                  /(St|Ave|Rd|Blvd|Dr|Ln|Way|Hwy|Ste|Suite|Street|Avenue|Road|Drive|Boulevard|Pkwy|Ct|Pl)\b/i.test(seg)) {
                address = seg;
                break;
              }
            }
            ...
            return { phone, address, city, state };
          }).catch(() => null);
          ...
          if (card?.phone) {
            ...
            if (isDuplicate?.(lead)) {
              ...
              onLog?.(`Collected ${leads.length} (card) — ${[card.phone && "phone", card.address && "address", cardWebsite && "website"].filter(Boolean).join(", ")}`);
            }
            continue; // skip the detail-panel click entirely
          }
        }
        ...
        await Promise.race([
          (async () => {
            ...
            await page.waitForFunction((bName: string) => {
              ...
              return false;
            }, businessName, { timeout: PANEL_WAIT_MS }).catch(() => null);
