...
/**
 * First-run browser setup. The desktop app no longer bundles Chromium (~650 MB);
 * it downloads it once, on the first scrape, into PLAYWRIGHT_BROWSERS_PATH. This
 * keeps the installer small. `playwright install chromium` is idempotent, so if the
 * browser is already present this is a fast no-op. A module-level promise ensures
 * concurrent scrapes trigger at most one download.
 */
let chromiumInstallPromise: Promise<void> | null = null;
async function ensureChromiumInstalled(): Promise<void> {
  if (findBundledChromium()) return; // already downloaded
  if (!chromiumInstallPromise) {
    chromiumInstallPromise = (async () => {
      const { execFile } = await import("node:child_process");
      const { promisify } = await import("node:util");
      const run = promisify(execFile);
      // The playwright CLI is copied into the bundle at assemble time; cwd is the
      // standalone dir in the packaged app (and the project root in dev).
      const cli = join(process.cwd(), "node_modules", "playwright", "cli.js");
      console.log("[scraper] Preparing scraper — downloading browser (first-run, one time)…");
      ...
      chromiumInstallPromise = null; // allow a retry on the next scrape
      throw e;
    });
  }
...
export async function launchScraperBrowser(): Promise<import("playwright-core").Browser> {
  ...
  return serialisedLaunch(async () => {
    if (isVercel) {
      ...
      // Download Chromium on first run if it isn't present yet (no-op if it is).
      await ensureChromiumInstalled();
      // When we're running the desktop browser, point Playwright straight at the
      // full Chromium exe so it never looks for chrome-headless-shell.
