...
import { launchScraperBrowser, createScraperContext } from "@/lib/scraping/crawler/core";
import { getSettings } from "@/lib/settings/service";

/**
 * Builds a LAZY email-grab context provider + its cleanup. The browser/context
 * is only created the first time a JS-rendered fallback is actually needed, so a
 * grab phase where every site resolves via plain fetch never launches a browser.
 * Reuses `sharedBrowser` when the caller already has one (cron batch runner).
 */
function makeGrabContextProvider(sharedBrowser?: import("playwright-core").Browser) {
  let browser: import("playwright-core").Browser | null = sharedBrowser ?? null;
  let owned = false;
  let context: import("playwright").BrowserContext | null = null;

  const getContext = async (): Promise<import("playwright").BrowserContext | null> => {
    if (context) return context;
    try {
      if (!browser) { browser = await launchScraperBrowser(); owned = true; }
      context = (await createScraperContext(browser)) as import("playwright").BrowserContext;
      return context;
    } catch {
      return null;
    }
  ...
  const cleanup = async (): Promise<void> => {
    ...
    // Only close the browser if WE launched it — a shared one belongs to the caller.
...
export async function processKeywordJob(
  ...
  // Route destination: if this keyword's category matches a real category on the
  // leads board, drop its leads into that category's shared "Ungrouped" folder so
  // they stay grouped under the category instead of becoming globally unfiled.
  // No matching category (or "Uncategorized") → leave unfiled as before.
  let categoryFolderId: string | undefined;
  try {
    const catName = (job.industry ?? "").trim();
    if (catName && catName.toLowerCase() !== "uncategorized") {
      ...
      if (industry) {
        const ownerId = kw?.createdById ?? industry.userId;
        const ungrouped = await getOrCreateUngroupedFolder(ownerId, industry.id, null);
        categoryFolderId = ungrouped.id;
      }
    }
  } catch { /* fall back to unfiled */ }
  ...
  let emailsGrabbed = 0;
  const grab = makeGrabContextProvider(sharedBrowser);
  if (pendingEmailGrabs.length > 0) {
    ...
    for (let gi = 0; gi < pendingEmailGrabs.length; gi++) {
      ...
      try {
        ...
        // A slightly higher cap allows the JS-render fallback to complete.
        const email = await Promise.race([
          grabEmailFromWebsite(website, { getContext: grab.getContext }),
...
// Per-keyword "dry streak": how many consecutive runs returned few NEW leads
// (duplicate-saturated / no fresh results). Drives adaptive backoff so a stuck
// keyword stops hogging a concurrency slot every cycle for near-zero gain, letting
// productive keywords cycle faster. Reset the moment a run is productive again.
const dryStreaks = new Map<string, number>();
    ...
    for (;;) {
      ...
      if (!active) {
        ...
        try {
          ...
          // How many NEW leads this run actually saved (drives adaptive backoff).
          const done = await prisma.scrapingJob
            .findUnique({ where: { id: newJob.id }, select: { leadsProcessed: true } })
            .catch(() => null);
          gainedThisRun = done?.leadsProcessed ?? 0;
        } catch (err) {
        ...
        // Track productivity. A run that adds few NEW leads means this keyword is
        // duplicate-saturated for its current query — so we lengthen the streak.
        const dryThreshold = Math.max(3, Math.ceil(kw.maxLeads * 0.1));
        if (gainedThisRun < dryThreshold) {
          dryStreaks.set(keywordId, (dryStreaks.get(keywordId) ?? 0) + 1);
        } else {
          dryStreaks.delete(keywordId);
        }
      }

      // Adaptive pause before the next cycle. Productive keywords barely pause
      // (just let DB writes settle). Saturated keywords back off exponentially —
      // 30s, 1m, 2m, 4m… capped at 10m — so they stop burning a concurrency slot
      // for almost no new leads and productive keywords get those slots instead.
      const streak = dryStreaks.get(keywordId) ?? 0;
      const pauseMs = streak === 0
        ? 1500
        : Math.min(10 * 60 * 1000, 30 * 1000 * 2 ** (streak - 1));
...
export async function processEmailRegrabJob(job: Awaited<ReturnType<typeof getJobById>>) {
  const id = job.id;
  const grab = makeGrabContextProvider();
  ...
  for (let i = 0; i < leads.length; i++) {
    ...
    try {
      const email = await Promise.race([
        grabEmailFromWebsite(lead.website!, { getContext: grab.getContext }),
        new Promise<null>((r) => setTimeout(() => r(null), 20_000)),
      ]);
      if (email) {
