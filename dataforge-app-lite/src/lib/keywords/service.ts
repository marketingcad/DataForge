...
import { getCitiesOfStateByPopulation } from "@/lib/geo/city-populations";
...
export function resolveRunLocation(kw: { location: string; cityIndex: number; cityRotationEnabled?: boolean }): {
  ...
  if (parts.length === 2) {
    ...
    // Largest population first, so early runs hit the dense markets instead of
    // burning the first dozen runs on whatever is alphabetically first.
    const cities = getCitiesOfStateByPopulation(country.isoCode, state.isoCode);
