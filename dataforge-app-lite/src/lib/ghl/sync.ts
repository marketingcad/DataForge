...
/** Duration (seconds) from a GHL call message's metadata. */
function messageDurationSecs(m: GhlMessage): number {
  if (typeof m.meta?.durationSecs === "number") return m.meta.durationSecs;
  if (typeof m.meta?.duration === "number") return Math.round(m.meta.duration / 1000);
  return 0;
}
...
export async function autoSyncGhlCalls(
  ...
  // Only conversations attributed to a known DataForge agent are worth expanding.
  const attributed = callConvs.filter((c) => c.assignedTo && agentByGhlId.has(c.assignedTo));
  const unmatched = callConvs.length - attributed.length;

  // Expand each conversation into its INDIVIDUAL call messages. GHL's own call
  // reports count one row per call *action* (not per contact thread), so we must
  // record one CallLog per call message — keyed on the real GHL message id.
  // Recording one row per conversation instead would collapse repeat calls to the
  // same contact and undercount; the old webhook path did the reverse and
  // overcounted. This is the single source of truth that matches GHL's numbers.
  const callsByConv = new Map<string, GhlMessage[]>();
  await runBatched(attributed, 10, async (conv) => {
    try {
      callsByConv.set(conv.id, await getConversationCalls(settings.ghlApiKey!, conv.id));
    } catch {
      callsByConv.set(conv.id, []);
    }
  });

  // Which message ids do we already have? (chunk the IN query to stay within limits.)
  const allMsgIds = [...callsByConv.values()].flatMap((msgs) => msgs.map((m) => m.id).filter(Boolean));
  const existingSet = new Set<string>();
  for (let i = 0; i < allMsgIds.length; i += 200) {
    const rows = await prisma.callLog.findMany({
      where: { ghlMessageId: { in: allMsgIds.slice(i, i + 200) } },
      select: { ghlMessageId: true },
    });
    for (const r of rows) if (r.ghlMessageId) existingSet.add(r.ghlMessageId);
  }

  let synced = 0;
  let skipped = 0;

  for (const conv of attributed) {
    const dfAgentId = agentByGhlId.get(conv.assignedTo!)!;
    const leadId = conv.contactId ? (contactToLeadId.get(conv.contactId) ?? null) : null;
    const msgs = callsByConv.get(conv.id) ?? [];

    await runBatched(msgs, 25, async (m) => {
      const ghlMessageId = m.id;
      if (!ghlMessageId) return;
      if (existingSet.has(ghlMessageId)) { skipped++; return; }
      existingSet.add(ghlMessageId);

      const calledAt = m.dateAdded ? new Date(m.dateAdded) : new Date();
      const direction = m.direction === "inbound" ? "inbound" : "outbound";
      const status = mapGhlCallStatus(String(m.status ?? ""));
      ...
      await prisma.callLog.upsert({
        ...
        create: {
          ...
          direction,
          durationSecs: messageDurationSecs(m),
          status,
          calledAt,
          ghlMessageId,
          notes: "Synced from GHL",
        },
        update: { calledAt, agentId: dfAgentId, leadId, direction, durationSecs: messageDurationSecs(m), status },
      });
      synced++;
    });
  }
