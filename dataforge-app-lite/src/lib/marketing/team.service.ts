...
import { startOfDayInTz, startOfMonthInTz, startOfWeekInTz } from "@/lib/utils/timezone";
...
function periodRange(
  ...
  const now = new Date();
  if (period === "today") return { gte: startOfDayInTz(tz, now) };
  if (period === "week")  return { gte: startOfWeekInTz(tz, now) };
  if (period === "month") return { gte: startOfMonthInTz(tz, now) };
  if (period === "custom" && customFrom) {
    const gte = startOfDayInTz(tz, customFrom);
    const lte = customTo
      ? new Date(startOfDayInTz(tz, customTo).getTime() + 86_400_000 - 1)
      : now;
    return { gte, lte };
  }
...
export const getTeamSummary = unstable_cache(async function getTeamSummary() {
  ...
  // Week/month boundaries in the configured timezone (to align with GHL). "Today"
  // stays a rolling 24-hour window on purpose.
  const tz             = (await getSettings().catch(() => null))?.timezone || "America/New_York";
  const startOfMonth   = startOfMonthInTz(tz, now);
  const startOfWeek    = startOfWeekInTz(tz, now);
...
export async function getLeaderboard(
  ...
  const tz = (await getSettings().catch(() => null))?.timezone || "America/New_York";
  const range = periodRange(period, tz, customFrom, customTo);
