...
import { startOfDayInTz, startOfWeekInTz, startOfMonthInTz } from "@/lib/utils/timezone";

// Philippine Standard Time is UTC+8; server runs in UTC. (Still used by the
// 6-month chart buckets below; the headline stats use the configured timezone.)
...
export async function getAgentStats(agentId: string) {
  ...
  const startOfDay   = startOfDayInTz(tz, now);
  ...
  const [total, today, thisWeek, thisMonth, last30, badges, activeTasks, appointmentsSet, dealsWon,
    durWeek, durMonth, durAll] = await Promise.all([
