...
export default function SignInPage() {
  ...
  const EMAIL_KEY = "df-last-email";
  const REMEMBER_FLAG = "df-remember-flag";

  // On load, always pre-fill the last-used email and restore the checkbox state.
  // (The password is never stored here — see note below.)
  useEffect(() => {
    try {
      const savedEmail = localStorage.getItem(EMAIL_KEY);
      if (savedEmail) setEmail(savedEmail);
      setRememberMe(localStorage.getItem(REMEMBER_FLAG) === "1");
  ...
  async function handleSubmit(e: React.SyntheticEvent<HTMLFormElement>) {
    ...
    // Always remember the email for next time; persist the checkbox state too.
    try {
