...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Call log write (DISABLED — API sync is the source of truth) ──────────
    // Call rows now come only from the GHL API sync (autoSyncGhlCalls), which
    // records one CallLog per real call message and matches GHL's reports. This
    // webhook keyed on a SYNTHETIC id (ghl-out:{userId}:{startTime}) that never
    // matched the sync's per-message ids, so keeping it created a second row for
    // every outbound call — the source of the 3x over-count. Flip WRITE_FROM_WEBHOOK
    // to true only if you also key it on GHL's real message id.
    const WRITE_FROM_WEBHOOK = false;
    if (WRITE_FROM_WEBHOOK && dedupKey) {
