...
export async function POST(req: NextRequest) {
  try {
    ...
    // wipe: delete every CallLog row first, then rebuild from GHL. Use this to fix a
    // corrupted/over-counted history — the API sync becomes the single source of truth
    // and the numbers will match GHL's own call reports. Only runs on the first batch
    // (cursor undefined) so pagination doesn't re-delete mid-rebuild.
    if (body.wipe && body.cursor == null) {
      await prisma.callLog.deleteMany({});
    }

    // forceReset/wipe: clear ghlCallsLastSyncedAt so we pull from the very beginning.
    if (body.forceReset || body.wipe) {
