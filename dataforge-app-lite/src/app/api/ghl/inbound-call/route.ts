...
export async function POST(req: NextRequest) {
  try {
    ...
    // Call logs are now sourced exclusively from the GHL API sync (autoSyncGhlCalls):
    // one CallLog per real call message, which matches GHL's own call reports exactly.
    // Webhook writes are disabled — they keyed on conversationId (a different dedup key
    // than the per-message sync) and produced duplicate / over-counted rows. Flip
    // WRITE_FROM_WEBHOOK to true only if you also unify the dedup key with the sync.
    const WRITE_FROM_WEBHOOK = false;
    if (WRITE_FROM_WEBHOOK && ghlMessageId) {
