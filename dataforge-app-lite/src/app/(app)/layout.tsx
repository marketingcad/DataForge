    ...
    <div className="flex h-screen overflow-hidden bg-background">
      <Suspense fallback={null}>
        <TabsProvider>
          ...
          <AppClientShell userName={session.user?.name} userEmail={session.user?.email} userId={session.user?.id ?? ""} role={role} forgerEnabled={!disabledFeatures.includes("forger")}>
            {children}
          </AppClientShell>
        </TabsProvider>
      </Suspense>
