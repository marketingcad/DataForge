...
export default async function HowItWorksPage() {
  ...
  const toc = [
    ...
    { id: "navigation",   label: "Navigation & Tips", icon: Sparkles },
      ...
      <div className="flex-1 min-w-0 space-y-8 pb-16">
        ...
        <Section id="workflow">
          ...
          <Card className="space-y-0 divide-y divide-border/40">
            {[
              { icon: ScanSearch,    color: "text-blue-500",    n: 1, title: "Discover leads via Scraping", desc: "Use Scrape a Website (crawl one domain) or Auto Keywords (Run now / Auto run against Google Maps) to pull business contacts into DataForge. Jobs run in the background. Duplicates are blocked before they enter the database." },
        ...
        <Section id="navigation">
          <SectionHeader icon={Sparkles} iconColor="bg-sky-500/15 text-sky-500" title="Navigation &amp; Tips" subtitle="Getting around DataForge quickly" />
          <div className="grid grid-cols-2 gap-3">
            <Card className="space-y-2.5">
              <SubLabel>Sidebar &amp; Tabs</SubLabel>
              <FeatureRow icon={LayoutGrid} iconColor="text-blue-500" title="Sidebar navigation" description="Every page is reached from the left sidebar (Dashboard, Leads, Scraping, Reports, etc.). Collapse it with the panel button — your choice is remembered." />
              <FeatureRow icon={LayoutGrid} iconColor="text-violet-500" title="Open in a new tab" description="Double-click a sidebar item — or hover it and click the small open-in-new-tab icon — to open that page in a browser-style tab. Tabs appear along the top; middle-click or the × closes one. Your open tabs are restored next time you launch the app." />
              <FeatureRow icon={ArrowRight} iconColor="text-emerald-500" title="Breadcrumb & back" description="The header shows the trail of pages you visited in the current tab (e.g. Dashboard › Leads › Scraping). Click any crumb, use the back arrow, or press Backspace to jump back." />
            </Card>
            <Card className="space-y-2.5">
              <SubLabel>Performance</SubLabel>
              <FeatureRow icon={Zap} iconColor="text-amber-500" title="Lite Mode" description="The feather icon in the top bar toggles Lite Mode — turns off animations for a snappier, lower-CPU/RAM experience on modest laptops. It's per-user and remembered across restarts." />
              <FeatureRow icon={Globe} iconColor="text-sky-500" title="On-demand globe" description="The Lead Origins globe on the Leads page loads its map data only when you click Show, so opening Leads stays fast even with tens of thousands of leads." />
              <p className="text-[11px] text-muted-foreground flex items-center gap-1.5 border-t border-border/40 pt-2">
                <Info className="h-3 w-3 shrink-0" />
                DataForge is a desktop app — it keeps running in the system tray when you close the window, so background scraping isn&apos;t interrupted. Quit fully from the tray menu.
              </p>
            </Card>
          </div>
        </Section>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Card className="space-y-3">
              ...
              <div className="space-y-2.5 pt-1 border-t border-border/40">
                ...
                <FeatureRow icon={FolderOpen} iconColor="text-blue-500"   title="Folders & Subcategories" description="Organise leads into named folders within a category, optionally grouped under subcategories. Rename, move, or delete anytime. Leads saved to a category without a specific folder land in that category's auto 'Ungrouped' folder (not lost as globally unfiled)." />
        ...
        <Section id="scraping" show={vis.scraping}>
          <SectionHeader icon={ScanSearch} iconColor="bg-amber-500/15 text-amber-500" title="Lead Scraping" subtitle="Two ways to automatically discover new leads" href="/scraping?tab=domain" />
          <div className="grid grid-cols-2 gap-3">
            {[
              {
              ...
              },
              {
                ...
                title: "Auto Keywords", access: "Boss · Admin · Lead Spec. (granted)", href: "/scraping?tab=keywords",
                desc: "Save keyword + location pairs, organised into category folders. Run one instantly with \"Run now\", or turn on \"Auto run\" to keep scraping Google Maps back-to-back until you stop it. Each folder has search + sort (name / most-fewest leads); leads land in the matching category.",
              },
            ].map((s) => (
