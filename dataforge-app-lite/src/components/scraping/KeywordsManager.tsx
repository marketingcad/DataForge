...
// Human-readable labels for the folder sort options.
const FOLDER_SORT_LABELS = {
  "name-asc": "Name (A–Z)",
  "name-desc": "Name (Z–A)",
  "leads-desc": "Most leads",
  "leads-asc": "Fewest leads",
} as const;
...
export function KeywordsManager({ initial, canManageAll = true, currentUserId = "" }: KeywordsManagerProps) {
  ...
  // How the folder cards are ordered.
  const [folderSort, setFolderSort] = useState<"name-asc" | "name-desc" | "leads-desc" | "leads-asc">("name-asc");
  ...
  useEffect(() => {
    try {
      ...
      // Restore the remembered folder sort order.
      const fs = localStorage.getItem("kw-folder-sort");
      if (fs && fs in FOLDER_SORT_LABELS) setFolderSort(fs as keyof typeof FOLDER_SORT_LABELS);
    } catch { /* ignore */ }
  }, []);
  ...
  // Total scraped leads per category (for the "by leads" sort).
  const leadsByCategory = new Map<string, number>(
    allCategoryNames.map((cat) => [
      cat,
      keywords
        .filter((k) => (k.category || "Uncategorized") === cat)
        .reduce((sum, k) => sum + (k._count?.leads ?? 0), 0),
    ])
  );

  // Folder cards in the user-chosen order.
  const sortedCategoryNames = [...allCategoryNames].sort((a, b) => {
    switch (folderSort) {
      case "name-desc":  return b.localeCompare(a);
      case "leads-desc": return (leadsByCategory.get(b) ?? 0) - (leadsByCategory.get(a) ?? 0);
      case "leads-asc":  return (leadsByCategory.get(a) ?? 0) - (leadsByCategory.get(b) ?? 0);
      case "name-asc":
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          {(keywords.length > 0 || manualCategories.length > 0) && (
            <Select
              ...
              onValueChange={(v) => {
                if (!v) return;
                setFolderSort(v as typeof folderSort);
                try { localStorage.setItem("kw-folder-sort", v); } catch { /* ignore */ }
              }}
            >
              <SelectTrigger className="h-8 w-[160px] text-sm">
                <span className="truncate">Sort: {FOLDER_SORT_LABELS[folderSort]}</span>
              </SelectTrigger>
              <SelectContent>
                {(Object.keys(FOLDER_SORT_LABELS) as (keyof typeof FOLDER_SORT_LABELS)[]).map((key) => (
                  <SelectItem key={key} value={key}>{FOLDER_SORT_LABELS[key]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          )}
      ...
      {(keywords.length > 0 || manualCategories.length > 0) && (
        <div className="flex flex-wrap gap-4">
          {sortedCategoryNames.map((cat, idx) => {
            const kwsInCat = keywords.filter((k) => (k.category || "Uncategorized") === cat);
...
function KeywordForm({
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      {/* Scheduling (Run schedule / interval) is hidden — the cron-based schedule
          is being retired in favour of Run now / Auto run. The interval prop is
          still submitted with its default so existing data stays valid. */}
      <div className="space-y-1.5">
