...
import { getFoldersAction, createFolderAction, getOrCreateUngroupedFolderAction } from "@/actions/folders.actions";
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (creatingNew || selectedFolder === "new") {
        ...
        // "None" means: if a category is selected in the filter, drop the leads
        // into that category's shared "Ungrouped" folder (so they stay inside the
        // category as an openable bucket). With no category, they're globally unfiled.
        if (filterIndustryId) {
          const ungrouped = await getOrCreateUngroupedFolderAction(filterIndustryId, filterSubcategoryId) as { id: string };
          folderId = ungrouped.id;
        } else {
          folderId = null;
        }
      } else {
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 flex flex-col min-w-0 p-5 gap-3">
            ...
            {loadingFolders ? (
              ...
              >
                {/* Unfiled / Ungrouped — becomes the category's "Ungrouped" bucket
                    when a category filter is active, so no-folder leads still land
                ...
                >
                  ...
                  <div className="min-w-0 w-full">
                    <p className="text-sm font-medium truncate">
                      {filterIndustry ? "Ungrouped" : "Unfiled"}
                    </p>
                    <p className="text-xs text-muted-foreground truncate">
                      {filterIndustry
                        ? `${filterIndustry.name}${filterSubcategory ? ` · ${filterSubcategory.name}` : ""}`
                        : "No folder"}
                    </p>
                  </div>
                </label>
