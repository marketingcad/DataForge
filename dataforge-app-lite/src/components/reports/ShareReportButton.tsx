...
// The public production site. A share link must be reachable by online
// recipients, so whenever this app is running locally (desktop app OR a local
// `npm run dev` browser — both have a localhost origin) we point the link at
// production instead. A real deployed origin (Vercel / custom domain) is used
// as-is. Overridable per-deploy via NEXT_PUBLIC_APP_URL.
const PUBLIC_BASE_URL = (
  process.env.NEXT_PUBLIC_APP_URL ?? "https://data-forge-nine.vercel.app"
).replace(/\/+$/, "");
...
export function ShareReportButton({ initialToken }: { initialToken: string | null }) {
  ...
  // Resolve the base once mounted: localhost → public production URL; a real
  // deployed origin → itself (so custom domains keep working).
  useEffect(() => {
    const origin = window.location.origin;
    setBase(isLocalOrigin(origin) ? PUBLIC_BASE_URL : origin);
  }, []);

  const link = token && base ? `${base}/share/reports/${token}` : "";
  ...
  function copyLink() {
    start(async () => {
      try {
        ...
        await navigator.clipboard.writeText(`${base}/share/reports/${t}`);
