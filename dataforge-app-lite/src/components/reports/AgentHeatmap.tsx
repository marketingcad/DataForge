...
export function AgentHeatmap({
  ...
  interactive?: boolean;
  /** YYYY-MM-DD the "Today" call column is pointed at (undefined = today). */
  ...
  const callDate = selectedCallDate ? new Date(`${selectedCallDate}T00:00:00`) : undefined;
  const pickCallDate = (d?: Date) => { if (d) router.push(`/reports?callDate=${format(d, "yyyy-MM-dd")}`); };
  const resetCallDate = () => router.push("/reports");
      ...
      <table className="w-full text-sm border-collapse min-w-[640px]">
        <thead>
          <tr className="border-b border-border/60">
            ...
            {columns.map((col) => {
              const isCallDate = variant === "calls" && col.key === "callsToday" && interactive;
              return (
                <th
                  key={String(col.key)}
                ...
                >
                  {isCallDate ? (
                    <Popover>
                      <PopoverTrigger
                        className="mx-auto inline-flex cursor-pointer items-center gap-1 rounded px-1.5 py-0.5 uppercase transition-colors hover:bg-muted hover:text-foreground"
                        title="Pick a date to see calls for that day"
                      >
                        <CalendarIcon className="h-3 w-3" />
                        {callDate ? format(callDate, "MMM d") : "Today"}
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0" align="center">
                        <Calendar mode="single" selected={callDate} onSelect={pickCallDate} />
                        <div className="flex items-center justify-between border-t p-2">
                          <button
                            type="button"
                            onClick={resetCallDate}
                            className="cursor-pointer text-xs font-medium text-primary hover:underline"
                          >
                            Reset to today
                          </button>
                          {callDate && (
                            <span className="text-[11px] normal-case text-muted-foreground">{format(callDate, "PPP")}</span>
                          )}
                        </div>
                      </PopoverContent>
                    </Popover>
